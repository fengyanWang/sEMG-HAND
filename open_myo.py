...
class Services(btle.Peripheral):
    ...
    def firmware(self):
        hex_fw = self.readCharacteristic(ReadHandle.FIRMWARE) # 发送获取固件版本信息的命令
        fw = struct.unpack('<4h', hex_fw)
        return fw

    # Get the battery level
    def battery(self):
        hex_batt = self.readCharacteristic(ReadHandle.BATTERY) #发送获取电池量的命令
        batt = ord(hex_batt)
        return batt
